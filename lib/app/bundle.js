...
//  modelling. Unfortunately there was little to no documentation on how to use the library -
//  though it is built on D3.js.
//  You can see it here http://bl.ocks.org/andredumas/a48008ea8e2c832144db
...
var _piechart = __webpack_require__(2);
...
$(document).ready(function () {
  ...
  $('#glossaryIcon').on("click", function (e) {
    e.preventDefault();
...
function BuildPiechart(url, category) {
  ...
  var labelArc = d3.arc().outerRadius(radius * 1.2).innerRadius(radius * 0.8);
  ...
  var data = d3.json(url, function (error, data) {
    ...
    var label = g.append("text").transition().ease(d3.easeLinear).duration(2500).attr("transform", function (d) {
      return "translate(" + labelArc.centroid(d) + ")";
    }).attr("dy", "14px").attr("transform", function (d) {
    ...
    }).style("fill", "none").style("stroke", "black").style("stroke-width", "1.5px");
